  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (enrichedRepos.length === 0) {
        return new Response(makeErrorSvg("No external contributions found in last 100 PRs"), { headers: { "Content-Type": "image/svg+xml" } });
      }
  ...
  let content = repos.map((repo, i) => {
    ...
    return `
      <g transform="translate(${x}, ${y})">
        ...
        <text x="55" y="20" font-family="-apple-system, BlinkMacSystemFont, Segoe UI, Helvetica, Arial, sans-serif" font-weight="600" font-size="14" fill="#0366d6" class="repo-name">${repo.name}</text>
